...
<div id="left_bar">
	...
	<div class="left_group">
		...
		<?php
		if (module_enabled('Forum')) : ?>
			<div class="left_link">
				<img src="<?=$mod->wwwPath('img/sidebar/menu_forum.gif')?>"/>
				::[<a href="<?=href('Forum', 'Boards')?>"><?=t('link_tbs_forum')?></a>]
				<?php
				$count = GDO_ForumUnread::countUnread($user); ?>
				<?php
				if ($count > 0) : ?>
					<div id="left-unread-forum" class="left-unread-count" <?=$count ? '' : 'style="display:none;"'?>><a
							href="<?=href('Forum', 'UnreadThreads')?>"><?=$count?></a></div>
				<?php
				else : ?>
		...
		if (module_enabled('PM')) : ?>
			<div class="left_link">
				...
				<?php
				if ($count > 0) : ?>
					<div id="left-unread-pm" class="left-unread-count" <?=$count ? '' : 'style="display:none;"'?>><a
							href="<?=href('PM', 'Overview')?>"><?=$count?></a></div>
				<?php
				else : ?>
